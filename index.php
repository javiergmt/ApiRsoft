...
try {
    ...
    if (
        ...
        throw new Exception("Metodo o servicio no autorizado");
    }
    ...
    //echo "\n";
    //echo "Llamando a servicio: " . $data['service'] . " metodo: " . $data['method'];
    //echo "\n";
    ...
    $Msg = $ex->getMessage();
    $Msg = ($Msg && stripos($Msg, 'SQLSTATE') === FALSE && stripos($Msg, 'schema') === FALSE && stripos($Msg, 'DATABASE') === FALSE) ? $Msg : 'Excepcion (ver error.log). Vuelva a intentar.';
    die(json_encode([
	"ok" => false,
